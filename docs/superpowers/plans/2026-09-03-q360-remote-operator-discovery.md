...
3. 在官方控制面关闭七个模式，恢复原始全局档位和原始 AI 目标温度，并刷新或重开页面；
4. 无论官方控制面显示什么，都让家中人员现场确认设备已停止发热、送风及其他运行；
5. 无法远程恢复或现场状态不明确时，由家中人员使用实体控制面停止设备，必要时按家中既有
   安全方式切断该设备供电；
6. 只记录固定错误码、阶段和明确时间戳；保留 incomplete 私有档案，不读取、输出或删除内容；
7. 不自动重试或继续原会话；再次实验必须重新预检并重新取得真实设备实验授权。

HA 已失联时不能调用 cancel，但仍须立即停止实验，通过官方控制面尝试恢复并通知现场人员；
恢复 HA 后只做只读核查。
```
