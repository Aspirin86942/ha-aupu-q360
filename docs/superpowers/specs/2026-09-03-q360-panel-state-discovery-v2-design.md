...
2. 复用当前认证 WSS、MQTT 解码和 Shadow `get`，观察完整目标设备 `reported`。
...
1. 验证 Config Entry 已加载、WSS 已订阅、未处于 Reauth/停止状态且无其他发现会话。
2. 若启用原始档案，创建权限受限的会话目录和 `.partial` 文件并启动写入任务。
3. 挂载 observer，使基线请求和响应都进入本次档案。
4. 发送带随机 token 的 Shadow `get`。
5. 在 10 秒内取得关联的完整目标 `reported`，作为“用户声明所有模式关闭”的会话基线。
...
- 启用原始档案但固定挂载不可用时，start 在网络请求前失败。
